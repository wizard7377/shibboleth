...
(** {2 Comment Tracking}
    ...
    Shared mutable state for tracking comments during lexing/parsing. The lexer
    populates this list as it encounters comments. After parsing, the frontend
    distributes comments to AST nodes based on source positions. *)
...
(** Drain all pending comments with position info, sorted by position. Used by
    the frontend post-parse pass to distribute comments to AST nodes. *)
...
let reset_comments () : unit = comment_store := []
