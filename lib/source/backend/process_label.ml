...
class process_label opts lexbuf =
  ...
  object (self)
    ...
    val comment_positions_by_text : (string, int list) Hashtbl.t =
      _comment_positions_by_text
    ...
    method private is_already_emitted (text : string) : bool =
      ...
      | Some positions ->
          List.exists
            (fun start ->
              StringSet.mem
                (self#make_dedup_key text start)
                emitted_comment_keys)
            positions
    (** Check if a comment text has already been emitted (by any path). Uses the
        position index for O(k) lookup where k = occurrences of this text. *)

    method private mark_emitted_at_pos (text : string) (pos : int) : unit =
      emitted_comment_keys <-
        StringSet.add (self#make_dedup_key text pos) emitted_comment_keys
    ...
    method cite : 'a. 'a citer -> string list -> 'a -> 'a =
      fun tag node_comments x ->
        ...
        total_comments_emitted <-
          total_comments_emitted + List.length node_comments;
        (* Filter out comments already emitted by leading_comments *)
        let fresh_comments =
          List.filter (fun c -> not (self#is_already_emitted c)) node_comments
        in
            ...
            | `Accumulate ->
                ...
                let with_positions =
                  List.map
                    (fun c ->
                      self#mark_emitted c;
                      (c, self#lookup_comment_position c))
                    fresh_comments
                in
                pending_comments <-
                  List.rev_append with_positions pending_comments;
                x (* Return unchanged *))
    ...
    *)
    ...
    method push_structure_boundary (pos : int) : int =
      ...
      last_structure_pos <- pos;
      saved
    (** Save last_structure_pos and set it to a new value. Returns the old value
        to restore later via [restore_structure_boundary]. Use when entering a
        nested scope (struct/sig body). *)

    method restore_structure_boundary (saved : int) : unit =
      last_structure_pos <- saved
    (** Restore last_structure_pos to a previously saved value. Use when leaving
        a nested scope. *)
      ...
      fun tag pos x ->
        ...
        | Some (start_pos, end_pos) -> (
            ...
            | _ ->
                ...
                total_comments_emitted <-
                  total_comments_emitted + List.length within;
                let attrs =
                  List.map
                    (fun c ->
                      self#mark_emitted c;
    ...
    (** Extract comments from the regex pool that fall within the given AST node
        position range, and attach them as attributes on the node using the
        citer. This is the primary mechanism for placing comments on
        expression/pattern nodes whose .comments field may be empty (because
        frontend distribution is coarse). *)

    method flush_all_remaining_as_structure_items :
        Parsetree.structure_item list =
    ...
    method comments_to_structure_items (node_comments : string list) :
        Parsetree.structure_item list =
      total_comments_seen <- total_comments_seen + List.length node_comments;
      (* Count all as emitted (deduped ones were already emitted by leading_comments) *)
      total_comments_emitted <-
    ...
    method check_all_comments_emitted : unit =
      if total_comments_seen > 0 && total_comments_emitted < total_comments_seen
      then
        failwith
          (Printf.sprintf
             "Comment conversion error: %d comments were seen but only %d were \
              emitted to the output OCaml. %d comment(s) were dropped."
