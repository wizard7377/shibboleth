...
module Trail : TRAIL = struct
  open! struct
    ...
    let rec suspend (trail, copy) =
      let rec suspend_prime = function
        | Nil -> Nil
        | Mark trail -> suspend_prime trail
        | Cons (action, trail) -> Cons (copy action, suspend_prime trail)
      in
      let ftrail = suspend_prime !trail in
      ref ftrail

    let rec resume (ftrail, trail, reset) =
      let rec resume_prime = function
        | Nil -> Nil
        | Mark ftrail -> resume_prime ftrail
        | Cons (faction, ftrail) -> Cons (reset faction, resume_prime ftrail)
      in
      let trail_prime = resume_prime !ftrail in
      trail := trail_prime

    let rec mark trail = trail := Mark !trail

    let rec unwind (trail, undo) =
      let rec unwind_prime = function
        | Nil -> Nil
        | Mark trail -> trail
