[@@@sml.comment {|(* Trailing Abstract Operations *)|}];;
[@@@sml.comment {|(* Author: Roberto Virga *)|}];;
        ...
        let rec suspend (trail, copy) =
          (let rec suspend_prime =
             (function 
                       ...
                       | (Cons (action, trail))
                           -> (Cons ((copy action), (suspend_prime trail))))
           in (let ftrail = (suspend_prime ((! trail))) in (ref ftrail)));;
        let rec resume (ftrail, trail, reset) =
          (let rec resume_prime =
             (function 
                       ...
                       | (Cons (faction, ftrail))
                           -> (Cons ((reset faction), (resume_prime ftrail))))
        ...
        let rec unwind (trail, undo) =
          (let rec unwind_prime =
             (function 
                       ...
                       | (Cons (action, trail))
                           -> begin
                                (undo action);(unwind_prime trail)
                                end)
