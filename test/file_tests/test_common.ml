...
module Make (M : S) = struct
    ...
    let resolve_path (path : Fpath.t) : Fpath.t =
        if Fpath.is_abs path then path
            ...
            | Some root ->
                    Fpath.v
                        (Filename.concat root (Fpath.to_string with_prefix))
    ...
    let strip_attributes (input : string) : string =
        ...
        let rec loop i =
            if i >= len then ()
            else if i + 2 < len && input.[i] = '[' && input.[i + 1] = '@' && input.[i + 2] = '@' then
    ...
    let normalize_output (input : string) : string =
        let stripped = strip_attributes input in
        let buf = Buffer.create (String.length stripped) in
        String.iter
            (fun ch ->
                if not (is_whitespace ch) then Buffer.add_char buf ch)
            stripped;
        Buffer.contents buf
    ...
    let run () =
        ...
        let run_with_context context_output =
            ...
            let expected = normalize_output (read_file M.test_expect) in
            let actual = normalize_output actual_raw in
        ...
        | Some expected_context ->
                ...
                write_actual_output ~expected_path:expected_context ~contents:actual_raw;
                let expected = normalize_output (read_file expected_context) in
                let actual = normalize_output actual_raw in
